...
##     use_exit_signa must set to true (or not set at all).                                             ##
...
class NostalgiaForInfinityNext(IStrategy):
    ...
    def whitelist_tracker(self):
        if sorted(self.coin_metrics['current_whitelist']) != sorted(self.dp.current_whitelist()):
            ...
            self.coin_metrics['current_whitelist'].insert(0, self.coin_metrics['current_whitelist'].pop(self.coin_metrics['current_whitelist'].index(f"BTC/{self.config['stake_currency']}:USDT")))
    ...
    def sell_profit_target(self, pair: str, trade: "Trade", current_time: "datetime", current_rate: float, current_profit: float, last_candle, previous_candle_1, previous_rate, previous_exit_reason, previous_time_profit_reached) -> tuple:
    ...
    def custom_exit(self, pair: str, trade: 'Trade', current_time: 'datetime', current_rate: float,
        ...
        if self.target_profit_cache is not None and pair in self.target_profit_cache.data:
            ...
            sell, signal_name = self.sell_profit_target(pair, trade, current_time, current_rate, current_profit, last_candle, previous_candle_1, previous_rate, previous_exit_reason, previous_time_profit_reached)
            if sell and signal_name is not None:
                return f"{signal_name} ( {enter_tag})"
        ...
        if self.sell_condition_1_enable and (last_candle['rsi_14'] > self.sell_rsi_bb_1) and (last_candle['close'] > last_candle['bb20_2_upp']) and (previous_candle_1['close'] > previous_candle_1['bb20_2_upp']) and (previous_candle_2['close'] > previous_candle_2['bb20_2_upp']) and (previous_candle_3['close'] > previous_candle_3['bb20_2_upp']) and (previous_candle_4['close'] > previous_candle_4['bb20_2_upp']) and (previous_candle_5['close'] > previous_candle_5['bb20_2_upp']):
            if (last_candle['close'] > last_candle['ema_200']):
                if (current_profit > 0.01):
                    return f"sell_signal_1_1_1 ( {enter_tag})"
            else:
                if (current_profit > 0.01):
                    return f"sell_signal_1_2_1 ( {enter_tag})"
                elif (max_loss > 0.5):
                    return f"sell_signal_1_2_2 ( {enter_tag})"
        ...
        # Sell signal 2
            ...
            if (last_candle['close'] > last_candle['ema_200']):
                if (current_profit > 0.01):
                    return f"sell_signal_2_1_1 ( {enter_tag})"
            else:
                if (current_profit > 0.01):
                    return f"sell_signal_2_2_1 ( {enter_tag})"
                elif (max_loss > 0.5):
                    return f"sell_signal_2_2_2 ( {enter_tag})"

        # Sell signal 3
        elif (self.sell_condition_3_enable) and (last_candle['rsi_14'] > self.sell_rsi_main_3):
            if (last_candle['close'] > last_candle['ema_200']):
                if (current_profit > 0.01):
                    return f"sell_signal_3_1_1 ( {enter_tag})"
            else:
                if (current_profit > 0.01):
                    return f"sell_signal_3_2_1 ( {enter_tag})"
                elif (max_loss > 0.5):
                    return f"sell_signal_3_2_2 ( {enter_tag})"
        ...
        # Sell signal 4
            ...
            if (last_candle['close'] > last_candle['ema_200']):
                if (current_profit > 0.01):
                    return f"sell_signal_4_1_1 ( {enter_tag})"
            else:
                if (current_profit > 0.01):
                    return f"sell_signal_4_2_1 ( {enter_tag})"
                elif (max_loss > 0.5):
                    return f"sell_signal_4_2_2 ( {enter_tag})"
        ...
        # Sell signal 6
            ...
            if (current_profit > 0.01):
                return f"sell_signal_6_1 ( {enter_tag})"
            elif (max_loss > 0.5):
                return f"sell_signal_6_2 ( {enter_tag})"

        # Sell signal 7
        elif self.sell_condition_7_enable and (last_candle['rsi_14_1h'] > self.sell_rsi_1h_7) and (last_candle['crossed_below_ema_12_26']):
            if (last_candle['close'] > last_candle['ema_200']):
                if (current_profit > 0.01):
                    return f"sell_signal_7_1_1 ( {enter_tag})"
            else:
                if (current_profit > 0.01):
                    return f"sell_signal_7_2_1 ( {enter_tag})"
                elif (max_loss > 0.5):
                    return f"sell_signal_7_2_2 ( {enter_tag})"

        # Sell signal 8
        elif self.sell_condition_8_enable and (last_candle['close'] > last_candle['bb20_2_upp_1h'] * self.sell_bb_relative_8):
            if (last_candle['close'] > last_candle['ema_200']):
                if (current_profit > 0.01):
                    return f"sell_signal_8_1_1 ( {enter_tag})"
            else:
                if (current_profit > 0.01):
                    return f"sell_signal_8_2_1 ( {enter_tag})"
                elif (max_loss > 0.5):
                    return f"sell_signal_8_2_2 ( {enter_tag})"
