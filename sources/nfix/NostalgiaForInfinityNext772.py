...
class NostalgiaForInfinityNext772(IStrategy):
    INTERFACE_VERSION = 2
    ...
    def load_hold_trades_config(self):
        if self.hold_trades_cache is None:
            hold_trades_config_file = NostalgiaForInfinityNext772.get_hold_trades_config_file()
    ...
    def custom_exit(self, pair: str, trade: 'Trade', current_time: 'datetime', current_rate: float,
        ...
        # Quick sell mode
        if all(c in ['32', '33', '34', '35', '36', '37', '38', '39', '40'] for c in enter_tags):
        ...
        if self.sell_condition_1_enable and (last_candle['rsi_14'] > self.sell_rsi_bb_1) and (last_candle['close'] > last_candle['bb20_2_upp']) and (previous_candle_1['close'] > previous_candle_1['bb20_2_upp']) and (previous_candle_2['close'] > previous_candle_2['bb20_2_upp']) and (previous_candle_3['close'] > previous_candle_3['bb20_2_upp']) and (previous_candle_4['close'] > previous_candle_4['bb20_2_upp']) and (previous_candle_5['close'] > previous_candle_5['bb20_2_upp']):
            if (last_candle['close'] > last_candle['ema_200']):
                if (current_profit > 0.0):
                    return 'sell_signal_1_1_1' + ' ( ' + enter_tag + ')'
            else:
                if (current_profit > 0.0):
                    return 'sell_signal_1_2_1' + ' ( ' + enter_tag + ')'
                elif (max_loss > 0.25):
                    return 'sell_signal_1_2_2' + ' ( ' + enter_tag + ')'
        ...
        # Sell signal 2
            ...
            if (last_candle['close'] > last_candle['ema_200']):
                if (current_profit > 0.0):
                    return 'sell_signal_2_1_1' + ' ( ' + enter_tag + ')'
            else:
                if (current_profit > 0.0):
                    return 'sell_signal_2_2_1' + ' ( ' + enter_tag + ')'
                elif (max_loss > 0.25):
                    return 'sell_signal_2_2_2' + ' ( ' + enter_tag + ')'
        ...
        # Sell signal 4
            ...
            if (last_candle['close'] > last_candle['ema_200']):
                if (current_profit > 0.0):
                    return 'sell_signal_4_1_1' + ' ( ' + enter_tag + ')'
            else:
                if (current_profit > 0.0):
                    return 'sell_signal_4_2_1' + ' ( ' + enter_tag + ')'
                elif (max_loss > 0.25):
                    return 'sell_signal_4_2_2' + ' ( ' + enter_tag + ')'
        ...
        # Sell signal 6
            ...
            if (current_profit > 0.0):
                return 'sell_signal_6_1' + ' ( ' + enter_tag + ')'
            elif (max_loss > 0.25):
                return 'sell_signal_6_2' + ' ( ' + enter_tag + ')'

        # Sell signal 7
        elif self.sell_condition_7_enable and (last_candle['rsi_14_1h'] > self.sell_rsi_1h_7) and (last_candle['crossed_below_ema_12_26']):
            if (last_candle['close'] > last_candle['ema_200']):
                if (current_profit > 0.0):
                    return 'sell_signal_7_1_1' + ' ( ' + enter_tag + ')'
            else:
                if (current_profit > 0.0):
                    return 'sell_signal_7_2_1' + ' ( ' + enter_tag + ')'
                elif (max_loss > 0.25):
                    return 'sell_signal_7_2_2' + ' ( ' + enter_tag + ')'

        # Sell signal 8
        elif self.sell_condition_8_enable and (last_candle['close'] > last_candle['bb20_2_upp_1h'] * self.sell_bb_relative_8):
            if (last_candle['close'] > last_candle['ema_200']):
                if (current_profit > 0.0):
                    return 'sell_signal_8_1_1' + ' ( ' + enter_tag + ')'
            else:
                if (current_profit > 0.0):
                    return 'sell_signal_8_2_1' + ' ( ' + enter_tag + ')'
                elif (max_loss > 0.25):
                    return 'sell_signal_8_2_2' + ' ( ' + enter_tag + ')'
