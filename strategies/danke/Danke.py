...
from freqtrade.strategy import merge_informative_pair, timeframe_to_minutes, DecimalParameter, IntParameter, informative
...
class Danke(IStrategy):
    INTERFACE_VERSION = 3
    plot_config = {'main_plot': {}, 'subplots': {'buy tag': {'enter_tag': {'color': 'green'}}}}
    # ROI table:
    minimal_roi = {'0': 10}
    stoploss = -0.99
    ...
    sell_custom_profit_bull_11 = 0.2
    ...
    sell_custom_profit_bear_11 = 0.2
    ...
    def load_hold_trades_config(self):
        if self.hold_trades_cache is None:
            hold_trades_config_file = NostalgiaForInfinity772martinsk2.get_hold_trades_config_file()
    ...
    def sell_over_main(self, current_profit: float, last_candle) -> tuple:
        if last_candle['close'] > last_candle['ema_200']:
            ...
            elif current_profit >= self.sell_custom_profit_bear_11:
    ...
    def sell_under_main(self, current_profit: float, last_candle) -> tuple:
        if last_candle['close'] < last_candle['ema_200']:
            ...
            elif current_profit >= self.sell_custom_under_profit_bear_11:
    ...
    def sell_under_min(self, current_profit: float, last_candle) -> tuple:
        ...
        # Uptrend
        elif current_profit >= self.sell_custom_profit_under_profit_2 and last_candle['close'] < last_candle['ema_200'] and ((last_candle['ema_200'] - last_candle['close']) / last_candle['close'] < self.sell_custom_profit_under_rel_2) and (last_candle['rsi_14'] > last_candle['rsi_14_1h'] + self.sell_custom_profit_under_rsi_diff_2):
    ...
    def normal_tf_indicators(self, dataframe: DataFrame, metadata: dict) -> DataFrame:
        ...
        dataframe['zema_200'] = ftt.zema(dataframe, period=100)
        ...
        dataframe['test'] = (dataframe['close'].rolling(48).max() >= dataframe['close'] * 1.125) & (dataframe['close'].rolling(288).max() >= dataframe['close'] * 1.225)
    ...
    def populate_entry_trend(self, dataframe: DataFrame, metadata: dict) -> DataFrame:
        ...
        for index in self.buy_protection_params:
            ...
            if self.buy_params['buy_condition_' + str(index) + '_enable']:
                ...
                item_buy_protection_list.append(dataframe['pump_strength'] < 0.15)
                item_buy_protection_list.append(dataframe['pump_strength'] > self.dump.value)
                item_buy_protection_list.append(dataframe['test'])
                item_buy_protection_list.append(dataframe[f'range{self.range_filter_length.value}'] < 1)
                dataframe['pump_strength'] < self.dump.value
                # Buy conditions
                # -----------------------------------------------------------------------------------------
                item_buy_logic = []
                ...
                # Condition #25
                    ...
                    item_buy_logic.append((dataframe['open'] < dataframe['ema_20_1h']) & (dataframe['low'] < dataframe['ema_20_1h']) | (dataframe['open'] > dataframe['ema_20_1h']) & (dataframe['low'] > dataframe['ema_20_1h']))
                ...
                # Condition #39 - Ichimoku
                    ...
                    # Start of trend
                    item_buy_logic.append((dataframe['leading_senkou_span_a_1h'].shift(12) < dataframe['leading_senkou_span_b_1h'].shift(12)) | (dataframe['ssl_up_1h'].shift(12) < dataframe['ssl_down_1h'].shift(12)))
    ...
    def confirm_trade_exit(self, pair: str, trade: 'Trade', order_type: str, amount: float, rate: float, time_in_force: str, exit_reason: str, **kwargs) -> bool:
        ...
        if self.holdSupportEnabled and self.config['runmode'].value in ('live', 'dry_run'):
            ...
            if exit_reason == 'force_exit':
                ...
                log.warning('Force selling %s even though the current profit of %s < %s', trade, formatted_current_profit_ratio, formatted_profit_ratio)
                return True
