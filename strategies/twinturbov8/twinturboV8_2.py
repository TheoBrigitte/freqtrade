...
class twinturboV8_2(IStrategy):
    ...
    def adjust_trade_position(self, trade: Trade, current_time: datetime,
        ...
        if current_profit > self.initial_safety_order_trigger:
            logger.info(f"{trade.pair} - Profit:{current_profit} Trigger:{self.initial_safety_order_trigger}")
            return None
    ...
    def custom_entry_price(self, pair: str, trade: Optional['Trade'], current_time: datetime, proposed_rate: float,
        ...
        entryprice = (dataframe['open'].iat[-1] + dataframe['close'].iat[-1] + proposed_rate) /3
        logger.info(f"{pair} using entryprice: {entryprice}")
    ...
    def populate_entry_trend(self, dataframe: DataFrame, metadata: dict) -> DataFrame:
        ...
        #         (dataframe[f'ma_buy_{self.base_nb_candles_buy.value}'] > dataframe[f'ma_sell_{self.base_nb_candles_sell.value}']) &
        #         (dataframe['HMA_SQZ'] < -0.5) &
        #         (dataframe['HMA_SQZ'] > dataframe['HMA_SQZ'].shift(1)) &
        #         (dataframe['close'] < dataframe['hma_50']) &
        #         # (dataframe['rsi'] < self.rsi_buy.value) &
        #         # (dataframe['BTC_EWO_Fast_1h'] >= self.bull.value) &
        #         (dataframe['volume'] > 0)
        #     ),
        #     ['enter_long', 'enter_tag']] = (1, 'HMA SQZ')
