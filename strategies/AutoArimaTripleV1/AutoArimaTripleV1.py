...
class AutoArimaTripleV1(IStrategy):
    ...
    log = open(os.path.join(os.path.dirname(os.path.realpath(__file__)), 'strategy.log'), 'w')
...
class ArimaPredictor(object):
    ...
    def __combine_data(self, c_data: DataFrame):
        ...
        self.data = self.data._append(c_data[c_data.index.isin(self.data.index) == False], verify_integrity=True)
