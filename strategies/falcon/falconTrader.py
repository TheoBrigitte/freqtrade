...
class falconTrader(IStrategy):
    ...
    buy_rmi = IntParameter(30, 50, default=35, optimize=True)
    buy_cci = IntParameter(-135, -90, default=-133, optimize=True)
    buy_srsi_fk = IntParameter(30, 50, default=25, optimize=True)
    buy_cci_length = IntParameter(25, 45, default=25, optimize=True)
    buy_rmi_length = IntParameter(8, 20, default=8, optimize=True)

    buy_bb_width = DecimalParameter(0.065, 0.135, default=0.095, optimize=True)
    buy_bb_delta = DecimalParameter(0.018, 0.035, default=0.025, optimize=True)
    
    buy_roc_1h = IntParameter(-25, 200, default=10, optimize=True)
    buy_bb_width_1h = DecimalParameter(0.3, 2.0, default=0.3, optimize=True)

    is_optimize_clucha = False
    buy_clucha_bbdelta_close = DecimalParameter(0.01,0.05, default=0.02206, optimize=True)
    buy_clucha_bbdelta_tail = DecimalParameter(0.7, 1.2, default=1.02515, optimize=True)
    buy_clucha_close_bblower = DecimalParameter(0.001, 0.05, default=0.03669, optimize=True)
    buy_clucha_closedelta_close = DecimalParameter(0.001, 0.05, default=0.04401, optimize=True)
    buy_clucha_rocr_1h = DecimalParameter(0.1, 1.0, default=0.47782, optimize=True)

    buy_ema_diff = DecimalParameter(0.022, 0.027, default=0.025, optimize=True)
    buy_bb_factor = DecimalParameter(0.990, 0.999, default=0.995, optimize=True)
    buy_closedelta = DecimalParameter(12.0, 18.0, default=15.0, optimize=True)
    ...
    sell_deadfish_bb_width = DecimalParameter(0.03, 0.75, default=0.05 , space='sell', optimize=True)
    sell_deadfish_profit = DecimalParameter(-0.15, -0.05, default=-0.08 , space='sell', optimize=True)
    sell_deadfish_bb_factor = DecimalParameter(0.90, 1.20, default=1.0 , space='sell', optimize=True)
    sell_deadfish_volume_factor = DecimalParameter(1, 2.5, default=1.5 ,space='sell', optimize=True)

    base_nb_candles_buy = IntParameter(8, 20, default=12, space='buy', optimize=True)
    base_nb_candles_sell = IntParameter(8, 50, default=22, space='sell', optimize=True)
    ...
    high_offset_2 = DecimalParameter(1.010, 1.020, default=1.01, space='sell', optimize=True)
    
    sell_trail_profit_min_1 = DecimalParameter(0.1, 0.25, default=0.1, space='sell', decimals=3, optimize=True)
    sell_trail_profit_max_1 = DecimalParameter(0.3, 0.5, default=0.4, space='sell', decimals=2, optimize=True)
    sell_trail_down_1 = DecimalParameter(0.04, 0.1, default=0.03, space='sell', decimals=3, optimize=True)

    sell_trail_profit_min_2 = DecimalParameter(0.04, 0.1, default=0.04, space='sell', decimals=3, optimize=True)
    sell_trail_profit_max_2 = DecimalParameter(0.08, 0.25, default=0.11, space='sell', decimals=2, optimize=True)
    sell_trail_down_2 = DecimalParameter(0.04, 0.2, default=0.015, space='sell', decimals=3, optimize=True)

    pHSL = DecimalParameter(-0.500, -0.040, default=-0.08, decimals=3, space='sell', optimize=True)
    ...
    pPF_2 = DecimalParameter(0.040, 0.100, default=0.080, decimals=3, space='sell',optimize=True)
    pSL_2 = DecimalParameter(0.020, 0.070, default=0.040, decimals=3, space='sell', optimize=True)
