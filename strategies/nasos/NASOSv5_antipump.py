...
# value loaded from strategy
# value loaded from strategy
buy_params = {'low_offset': 0.981, 'base_nb_candles_buy': 8, 'ewo_high': 3.553, 'ewo_high_2': -5.585, 'ewo_low': -14.378, 'lookback_candles': 32, 'low_offset_2': 0.942, 'profit_threshold': 1.037, 'rsi_buy': 78, 'rsi_fast_buy': 37}
# Sell hyperspace params:
# value loaded from strategy
# value loaded from strategy
# value loaded from strategy
sell_params = {'base_nb_candles_sell': 16, 'high_offset': 1.097, 'high_offset_2': 1.472}
...
class NASOSv5(IStrategy):
    ...
    stoploss = -0.15
    # SMAOffset
    base_nb_candles_buy = IntParameter(2, 20, default=buy_params['base_nb_candles_buy'], space='buy', optimize=False)
    base_nb_candles_sell = IntParameter(2, 25, default=sell_params['base_nb_candles_sell'], space='sell', optimize=False)
    ...
    profit_threshold = DecimalParameter(0.99, 1.05, default=buy_params['profit_threshold'], space='buy', optimize=False)
    ...
    rsi_fast_buy = IntParameter(10, 50, default=buy_params['rsi_fast_buy'], space='buy', optimize=False)
    # Trailing stop:
    trailing_stop = False
    ...
    #                'mfi': {'color': 'blue'},
    plot_config = {'main_plot': {'ma_buy_8': {'color': 'orange'}, 'ma_sell_16': {'color': 'orange'}}, 'subplots': {'rsi': {'rsi': {'color': 'orange'}, 'rsi_fast': {'color': 'red'}, 'rsi_slow': {'color': 'green'}}, 'ewo': {'EWO': {'color': 'blue'}}, 'ps': {'pump_strength': {'color': 'yellow'}}}}
...
class NASOSv5_antipump(NASOSv5):
    ...
    def populate_entry_trend(self, dataframe: DataFrame, metadata: dict) -> DataFrame:
        ...
        # don't buy if there isn't 3% profit to be made
        dont_buy_conditions.append(dataframe['close_15m'].rolling(self.lookback_candles.value).max() < dataframe['close'] * self.profit_threshold.value)
        dataframe.loc[(dataframe['pump_strength'] < self.antipump_threshold.value) & (dataframe['rsi_fast'] < self.rsi_fast_buy.value) & (dataframe['close'] < dataframe[f'ma_buy_{self.base_nb_candles_buy.value}'] * self.low_offset.value) & (dataframe['EWO'] > self.ewo_high.value) & (dataframe['rsi'] < self.rsi_buy.value) & (dataframe['volume'] > 0) & (dataframe['close'] < dataframe[f'ma_sell_{self.base_nb_candles_sell.value}'] * self.high_offset.value), ['enter_long', 'enter_tag']] = (1, 'ewo1')
        dataframe.loc[(dataframe['rsi_fast'] < self.rsi_fast_buy.value) & (dataframe['close'] < dataframe[f'ma_buy_{self.base_nb_candles_buy.value}'] * self.low_offset_2.value) & (dataframe['EWO'] > self.ewo_high_2.value) & (dataframe['rsi'] < self.rsi_buy.value) & (dataframe['volume'] > 0) & (dataframe['close'] < dataframe[f'ma_sell_{self.base_nb_candles_sell.value}'] * self.high_offset.value) & (dataframe['rsi'] < 25), ['enter_long', 'enter_tag']] = (1, 'ewo2')
        dataframe.loc[(dataframe['rsi_fast'] < self.rsi_fast_buy.value) & (dataframe['close'] < dataframe[f'ma_buy_{self.base_nb_candles_buy.value}'] * self.low_offset.value) & (dataframe['EWO'] < self.ewo_low.value) & (dataframe['volume'] > 0) & (dataframe['close'] < dataframe[f'ma_sell_{self.base_nb_candles_sell.value}'] * self.high_offset.value), ['enter_long', 'enter_tag']] = (1, 'ewolow')
        if dont_buy_conditions:
            for condition in dont_buy_conditions:
